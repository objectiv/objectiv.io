...
* **`func`** (_Union_[_str_, _Callable_, _List_[_Union_[_Callable_, _str_]_]_]) – the aggregation function to look for on all series.
See GroupBy.agg() for supported arguments

* **`group_by`** (_GroupBy) – the group_by to use, or aggregation over full base_node if None
