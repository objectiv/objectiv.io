...
* **`df`** (_pandas.core.frame.DataFrame) – Pandas DataFrame to instantiate as DataFrame.

* **`convert_objects`** (_bool) – If True, the data in the columns with dtypes not in the list of supported
...
* **`name`** (_str) –   * For ‘table’ materialization: name of the table that Pandas will write the data to.

  * For ‘cte’ materialization: name of the node in the underlying SqlModel graph.


* **`materialization`** (_str) – {‘cte’, ‘table’}. How to materialize the data.

* **`if_exists`** (_str) – {‘fail’, ‘replace’, ‘append’}. Only applies to `materialization=’table’`
