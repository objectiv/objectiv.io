...
<h2 className="signature-method"><span className="type-annotation"><em>classmethod</em></span> <span className="additional-name">DataFrame.</span><span className="name">from_pandas(</span><small className="parameter-list"><span className="parameter" id="engine">engine</span>, <span className="parameter" id="df">df</span>, <span className="parameter" id="convert_objects">convert_objects</span>, <span className="parameter" id="name">name='loaded_data'</span>, <span className="parameter" id="materialization">materialization='cte'</span>, <span className="parameter" id="if_exists">if_exists='fail'</span></small>)</h2>
...
[bach.dataframe.DataFrame](/docs/modeling/bach/api-reference/DataFrame/bach.DataFrame.from_pandas/)
