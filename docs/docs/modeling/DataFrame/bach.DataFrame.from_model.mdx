---
id: bach.DataFrame.from_model
title: bach.DataFrame.from_model — Bach  documentation
hide_title: true
slug: /modeling/DataFrame/bach.DataFrame.from_model


---
<head>
    <meta name="description" content="Instantiate a new DataFrame based on the result of the query defined in model." />
    <meta property="og:description" content="Instantiate a new DataFrame based on the result of the query defined in model." />
</head>

export const toc = [];


import SphinxPages from '@site/src/components/sphinx-page'
import useBaseUrl from '@docusaurus/useBaseUrl'


<SphinxPages url={useBaseUrl('modeling/DataFrame/bach.DataFrame.from_model.html')} />
