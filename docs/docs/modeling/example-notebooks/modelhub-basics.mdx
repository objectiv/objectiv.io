...
title: Open model hub basics
---

# Open model hub basics

In this example, we briefly demonstrate how you can use pre-built models from the open model hub in
conjunction with, [Bach](/modeling/bach/index.mdx), our modeling library.

This example uses real, unaltered data that was collected from [https://objectiv.io](https://objectiv.io) with Objectiv’s Tracker. All models
in the open model hub are compatible with data sets that have been validated against the
[open analytics taxonomy](https://objectiv.io/docs/taxonomy/).

This example is also available in a [notebook](https://github.com/objectiv/objectiv-analytics/blob/main/notebooks/model-hub-demo-notebook.ipynb)
...
1. [Helper functions](/modeling/open-model-hub/models/helper-functions/index.mdx). These helper functions
simplify manipulating and
analyzing the data.
2. [Aggregation models](/modeling/open-model-hub/models/aggregation/index.mdx). These models consist of a
combination of Bach
...
at the [logistic regression example](/modeling/example-notebooks/logistic-regression.mdx).
...
(this is called the location stack, see [location_stack](/modeling/example-notebooks/open-taxonomy.mdx#location-stack) for info).
...
There is another [example](/modeling/bach/examples.mdx) that demonstrates what you can do with the Bach modeling
library, or head over to the [Bach API reference](/modeling/bach/api-reference/index.mdx#bach-api-reference) for a complete overview of the possibilities.
...
For a complete overview of all available and upcoming models, check out the [models](/modeling/open-model-hub/models/index.mdx).
