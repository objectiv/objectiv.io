...
validated against the [open analytics taxonomy](https://objectiv.io/docs/taxonomy/). It’s also available in
a [notebook](https://github.com/objectiv/objectiv-analytics/blob/main/notebooks/open-taxonomy-how-to.ipynb)
...
<div id="open-taxonomy-location-stack-and-global-contexts" className="hidden-anchor"></div>
...
In case a json array does not contain the object, `None` is returned. More info at the [API reference](/modeling/bach/api-reference/Series/Json/index.mdx).
...
* All methods of [SeriesJson](/modeling/bach/api-reference/Series/Json/index.mdx) for the json type can also be accessed using `.ls`
...
* [.gc.get_from_context_with_type_series(type, key)](/modeling/open-model-hub/api-reference/SeriesGlobalContexts/modelhub.SeriesGlobalContexts.objectiv.mdx).
* The property accessors:
...
* All methods of [SeriesJson](/modeling/bach/api-reference/Series/Json/index.mdx) for the json type can also be accessed using `.gc`
