...
This example is also available in a [notebook](https://github.com/objectiv/objectiv-analytics/blob/main/notebooks/basic-analysis.ipynb)
...
[Getting started with Objectiv](/docs/modeling/example-notebooks/product-analytics/) for more info on this. The data used in this example is
...
[get_from_context_with_type_series](/docs/modeling/open-model-hub/api-reference/SeriesLocationStack/modelhub.SeriesLocationStack.objectiv). Or use methods
specific to the [location_stack](/docs/modeling/example-notebooks/open-taxonomy) or [global_contexts](/docs/modeling/example-notebooks/open-taxonomy) to
extract the data.
...
Now we will go though a bunch of basic analytics metrics. We can use models from the [models](/docs/modeling/open-model-hub/models)
for this purpose or use [Bach](/docs/modeling/bach) to do data analysis directly on the data stored in the SQL database using
pandas-like syntax.
