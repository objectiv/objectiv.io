---
id: get-started-in-your-notebook
sidebar_position: 5
slug: /modeling/get-started-in-your-notebook/
title: Get started in your notebook
---

# Get started in your notebook

You can run Objectiv in your favorite notebook. From a self-hosted Jupyter to the cloud with
[Google Colab](https://colab.research.google.com/), [Hex](https://hex.tech/) and
[Deepnote](https://deepnote.com).

Here’s how you can use Objectiv in these different notebooks:

## Jupyter

1. Install the open model hub locally:
```console
pip install objectiv-modelhub
```

1. Import the required packages, at the start of your notebook:
```python
from modelhub import ModelHub
```

1. Instantiate the model hub and set the default time aggregation:
```python
modelhub = ModelHub(time_aggregation='%Y-%m-%d')
```

1. Setup the db connection (optionally first create a local SSH tunnel)
```python
df = modelhub.get_objectiv_dataframe(db_url='postgresql://USER:PASSWORD@HOST:PORT/DATABASE',
        start_date='2022-06-01',
        end_date='2022-06-30',
        table_name='data')
```

## Google Colab / Hex / Deepnote

1. Install the open model hub at the start of your notebook:
```console
pip install objectiv-modelhub
```

1. Import the required packages, next in your notebook:
```python
from modelhub import ModelHub
```

1. Instantiate the model hub and set the default time aggregation:
```python
modelhub = ModelHub(time_aggregation='%Y-%m-%d')
```

1. Optionally: create an SSH tunnel to the Postgres database server:
```python
pip install sshtunnel
```

```python
from sshtunnel import SSHTunnelForwarder
import os, stat

# SSH tunnel configuration
ssh_host = ''
ssh_port = 22
ssh_username = ''
ssh_passphrase = ''
ssh_private_key= ''
db_host = ''
db_port = 5432

try:
    pk_path = '._super_s3cret_pk1'
    with open(pk_path, 'a') as pkf:
        pkf.write(ssh_private_key)
        os.chmod(pk_path, stat.S_IREAD)

    ssh_tunnel = SSHTunnelForwarder(
        (ssh_host, ssh_port),
        ssh_username=ssh_username,
        ssh_private_key=pk_path,
        ssh_private_key_password=ssh_passphrase,
        remote_bind_address=(db_host, db_port)
    )
    ssh_tunnel.start()
    os.remove(pk_path)
    tunnel_port = ssh_tunnel.local_bind_port

except Exception as e:
    os.remove(pk_path)
    raise(e)
```

1. Setup the db connection
```python
df = modelhub.get_objectiv_dataframe(
    db_url=f'postgresql://USER:PASSWORD@localhost:{tunnel_port}/DATABASE',
    start_date='2022-06-01',
    end_date='2022-06-30',
    table_name='data')
```

*For Deepnote specifically:*
as very first step: create a requirements.txt, add below and restart the machine:

```python
pandas==1.4.1
```

## Next steps

After these steps, you’re ready to go! Check out the [example notebooks](/modeling/index.mdx) and the
[open model hub](/modeling/open-model-hub/index.mdx) for where to take this next.
