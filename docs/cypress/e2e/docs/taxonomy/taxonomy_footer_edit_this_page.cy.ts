describe('Docs Taxonomy: Footer Edit This Page', () => {
  it('Should track PressEvents on Edit This Page items', () => {
    ...
    cy.objectiv().filterEvents('PressEvent').its('events').should('have.length', 3).then(recordedEvents => {
      ...
      cy.wrap(root).should('deep.equal', {_type: 'RootLocationContext', id: 'taxonomy'});
      ...
      cy.wrap(link).its('href').should('eq', 'https://github.com/objectiv/objectiv.io/edit/main/docs/docs/taxonomy/introduction.md');
    });
